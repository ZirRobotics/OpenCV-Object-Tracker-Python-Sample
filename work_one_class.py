...
def detect_objects(model, frame, detection_queue, target_class=0):
    """
    Perform object detection and put the results in the detection_queue.
    Only detections of the target_class are considered.

    :param model: YOLOv8 model
    :param frame: Image frame for detection
    :param detection_queue: Queue to put detection results
    :param target_class: Class index to filter detections (default is 0 for 'person')
    """
    try:
        # Perform detection with class filtering if supported
        # Uncomment the following line if 'classes' parameter is supported
        # results = model(frame, classes=[target_class])

        # Alternatively, perform detection without class filtering and filter manually
        results = model(frame)
        bboxes = []
        ...
        logging.debug(f"Detected {len(bboxes)} 'person' bounding boxes.")
        ...
        for bbox in detected_bboxes:
            ...
            if tracker is not None:
                try:
                    ...
                    logging.error(
                        f"Exception during tracker initialization for '{tracker_algorithm}' with bbox {bbox}: {e}")
            else:
...
def main():
    ...
    logging.info(f"Video resolution: {int(actual_width)}x{int(actual_height)}")

    # Load YOLOv8 model ######################################################
    try:
        # model_path = "/home/artem-n/PycharmProjects/model/fly_last.pt"
    ...
    def detection_worker():
        while not stop_event.is_set():
            try:
                ...
                detect_objects(model, frame, detection_queue, target_class=0)  # Specify target_class here
    ...
    try:
        while cap.isOpened():
            ...
            try:
                ...
                if detected_bboxes:
                    logging.debug(f"Retrieved {len(detected_bboxes)} 'person' bounding boxes from detection_queue.")
            ...
            if k == 32:  # SPACE
                ...
                try:
                    ...
                    if detected_bboxes:
                        tracker_list = initialize_tracker_list(image, tracker_algorithm_list, detected_bboxes)
                        logging.info(f"Reinitialized {len(tracker_list)} trackers based on new detections.")
                except queue.Empty:
                    logging.info("No new detections available to reinitialize trackers.")
            if k == 27:  # ESC
